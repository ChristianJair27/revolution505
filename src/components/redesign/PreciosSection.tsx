    ...
    >
      ...
      <div className={`flex flex-col flex-1 p-6 md:p-8 ${plan.highlight ? 'bg-surface' : 'bg-surface/40'}`}>
        ...
        <div className="space-y-2.5">
          ...
          <a
            href={`${CONTACT_MAILTO}?subject=${encodeURIComponent(`Cotización ${plan.name} — Revolution505`)}&body=${encodeURIComponent(`Hola, me interesa el plan ${plan.name}.\n\nNombre:\nNegocio:\n`)}`}
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-full text-xs text-muted hover:text-text-primary border border-transparent hover:border-stroke transition-colors"
          >
            <Mail className="w-3.5 h-3.5" />
            O por correo ({CONTACT_EMAIL})
          </a>
        </div>
      </div>
    </motion.div>
    ...
    >
      <div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-10">
        <div className="flex-1 space-y-4">
          <div>
            <p className="text-sm font-medium text-text-primary mb-1">🧮 Calculadora de retorno de inversión</p>
            <p className="text-xs text-muted">Plan seleccionado: <span className="text-accent">{selectedPlan.emoji} {selectedPlan.name} (${fmt(selectedPlan.price)} MXN)</span></p>
          </div>
          <div>
            <p className="text-xs text-muted mb-2">¿Cuánto cobra por cliente en promedio?</p>
            <div className="flex flex-wrap gap-2">
              {TICKET_OPTIONS.map(opt => (
                <button key={opt.value} onClick={() => setTicket(opt.value)} className={`px-3 py-1.5 rounded-full border text-xs transition-all duration-200 ${
                    ticket === opt.value ? 'border-accent/60 bg-accent/10 text-text-primary' : 'border-stroke text-muted hover:border-stroke/70 hover:text-text-primary'
                  }`}>{opt.label}</button>
              ))}
            </div>
          </div>
        ...
        <div className="flex-shrink-0 text-center md:text-right">
          <p className="text-[11px] text-muted uppercase tracking-wider mb-1">Consiguiendo solo {NEW_CLIENTS} clientes nuevos/mes</p>
          <AnimatePresence mode="wait">
            <motion.p key={`${ticket}-${selectedPlan.id}`} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.2 }} className="text-4xl md:text-5xl font-display italic accent-gradient-text leading-none">
              {monthsToRecover === 1 ? '1 mes' : `${monthsToRecover} meses`}
            </motion.p>
          </AnimatePresence>
          <p className="text-xs text-muted mt-1">para recuperar tu inversión</p>
          <p className="text-[10px] text-muted/60 mt-2 max-w-[180px] md:ml-auto">Cálculo conservador. Muchos clientes lo recuperan en menos tiempo.</p>
        </div>
      </div>
    </motion.div>
  )
...
const TRUST_POINTS = ['Revisiones incluidas', 'Sin cargos ocultos', 'Soporte post-entrega']

function GuaranteeSection() {
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.6 }} className="mt-10 rounded-3xl border border-emerald-500/25 overflow-hidden" style={{ background: 'linear-gradient(135deg, hsl(160 60% 10% / 0.6) 0%, hsl(220 30% 8% / 0.8) 100%)' }}>
      ...
      <div className="p-6 md:p-8 flex flex-col md:flex-row items-center gap-6 md:gap-8">
        <div className="flex-shrink-0 w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: 'hsl(160 60% 30% / 0.15)', border: '1px solid hsl(160 60% 50% / 0.25)' }}>
          <Shield className="w-8 h-8" style={{ color: 'hsl(160 60% 55%)' }} strokeWidth={1.5} />
        </div>
        <div className="flex-1 text-center md:text-left">
          <h4 className="text-base font-semibold text-text-primary mb-1">Garantía de Satisfacción 100%</h4>
          <p className="text-sm text-muted leading-relaxed max-w-md">Si el diseño inicial no te convence, lo rehacemos sin costo adicional. Tu inversión está protegida desde el primer día.</p>
        </div>
        <div className="flex-shrink-0 flex flex-row md:flex-col gap-4 md:gap-2">
          {TRUST_POINTS.map(item => (
            <div key={item} className="flex items-center gap-2 text-xs whitespace-nowrap" style={{ color: 'hsl(160 60% 55%)' }}>
              <Check className="w-3.5 h-3.5 flex-shrink-0" strokeWidth={2.5} />{item}
            </div>
          ))}
        </div>
      </div>
...
export default function PreciosSection() {
  ...
  return (
    <section ref={sectionRef} id="precios" className="relative py-24 md:py-32 px-4 border-t border-stroke overflow-hidden">
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[600px] h-[300px] blur-3xl pointer-events-none" style={{ background: 'radial-gradient(ellipse, hsl(217 91% 60% / 0.07), transparent 70%)' }} aria-hidden />
      <div className="max-w-6xl mx-auto relative">
        <div className="precio-header text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-3">
            <span className="w-8 h-px bg-stroke" /><span className="text-xs text-muted uppercase tracking-[0.3em]">Inversión</span><span className="w-8 h-px bg-stroke" />
          </div>
          <h2 className="text-3xl md:text-5xl font-display italic text-text-primary">Planes{' '}<em className="not-italic accent-gradient-text">transparentes</em></h2>
          <p className="text-sm text-muted mt-3 max-w-lg mx-auto leading-relaxed">Precios fijos. Sin letra pequeña, sin cargos ocultos. Hosting, dominio y soporte incluidos en todos los planes web.</p>
        </div>
        <div className="precio-toggle flex justify-center mb-12">
          <div className="inline-flex items-center gap-1 p-1 rounded-full border border-stroke bg-surface/50">
            {(['web', 'sistemas'] as TabType[]).map(t => (
              <motion.button key={t} onClick={() => setTab(t)} className={`relative px-5 py-2 rounded-full text-sm transition-colors duration-200 ${
                  tab === t ? 'text-text-primary' : 'text-muted hover:text-text-primary'
                }`}>
                {tab === t && (<motion.span layoutId="tab-bg" className="absolute inset-0 rounded-full bg-stroke/60" transition={{ type: 'spring', bounce: 0.2, duration: 0.35 }} />)}
                <span className="relative z-10">{t === 'web' ? '💼 Proyectos Web' : '⚙️ Sistemas'}</span>
              </motion.button>
            ))}
          </div>
        </div>
        <div className="precio-grid">
          <AnimatePresence mode="wait">
            <motion.div key={tab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="grid md:grid-cols-3 gap-5 md:gap-6 md:items-start">
        ...
        <GuaranteeSection />
        <motion.div initial={{ opacity: 0, y: 12 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-40px' }} transition={{ duration: 0.5, delay: 0.15 }} className="mt-6 flex justify-center">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <a href={WA.proyectoMedida} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-2 px-6 py-3 rounded-full border border-stroke text-sm text-muted hover:text-text-primary hover:border-accent/40 transition-all duration-200">
              ¿Proyecto a medida? WhatsApp<ArrowRight className="w-4 h-4 group-hover:translate-x-0.5 transition-transform" />
            </a>
            <a href={`${CONTACT_MAILTO}?subject=${encodeURIComponent('Proyecto a medida — Revolution505')}`} className="flex items-center gap-2 px-6 py-3 rounded-full text-sm text-muted hover:text-text-primary transition-colors">
              <Mail className="w-4 h-4" />O escribe a {CONTACT_EMAIL}
            </a>
          </div>
        </motion.div>
      </div>
